...
// TODO review all clones and check if necessary or if the data structures can be reformulated as
...
fn eval(term: Term) -> EvalResult {
    ...
    let mut fresh_identifier = move || {
        let res = Identifier(format!("var{}", cur_identifier));
        cur_identifier += 1;
        res
    };
    ...
    loop {
        let newconf = match conf {
            ...
            Conf::Up(
                ...
                let mut e_: Env = Env(e.0.clone()); // TODO fix / make Env persistent
